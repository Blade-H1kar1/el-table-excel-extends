  ...
  try {
    ...
    for (const item of clipboardItems) {
      if (item.types.includes(applicationType)) {
        ...
        if (jsonData.type === "super-crud-data" && jsonData.data) {
          return { textData: jsonData.data.textData, isInternal: true };
        }
      }
    }
    ...
    for (const item of clipboardItems) {
      if (item.types.includes("text/plain")) {
        ...
        return { textData, isInternal: false };
      }
    }
...
// 树形数据转换为数组
export function toTreeArray(tree, options = {}) {
  // 参数校验
  ...
  function traverse(nodes, level = 0, parent = null) {
    ...
    nodes.forEach((node, index) => {
      ...
      // 创建节点副本，避免修改原始数据
      const nodeItem = config.strict ? { ...node } : node;
      ...
      if (Array.isArray(children) && children.length > 0) {
        // 如果有自定义的子节点映射函数
        const childrenToProcess = config.mapChildren
          ? children.map(config.mapChildren)
          : children;
